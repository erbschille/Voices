...
dp1_b <- d_p1 %>% 
  ...
  filter(question_text %in% c("Have you ever heard God or a spirit speak to you in a voice you felt you heard outside your head?",
                              "Have you heard God or a spirit speak to you in a voice that you felt you experienced inside your head?")) %>%
  group_by(subject_id) %>%
  mutate(voice_score = sum(response, na.rm = TRUE)) %>% #getting spiritual voice score for each person 
...
# Launay Slade scores 
dp2_b <- d_p2 %>% 
  filter(subscale_name == "Hallucination: Auditory") %>% 
  group_by(subject_id) %>%
...
## DIALOGIC subscale
m.dp2_d <- lm(
  scale(ls_score, scale = T) ~ 1 + scale(visq_score_d, scale = T) * country, 
  data = dp2_small)
summary(m.dp2_d)

##OTHER PEOPLE subscale
# m.dp2_o <- lm(
#   scale(ls_score, scale = T) ~ 1 + scale(visq_score_o, scale = T) * country, 
#   data = dp2_small)
# summary(m.dp2_o)

## EVALUATIVE subscale
# m.dp2_e <- lm(
#   scale(ls_score, scale = T) ~ 1 + scale(visq_score_e, scale = T) * country, 
#   data = dp2_small)
# summary(m.dp2_e)
```
...
```{r}
m.voice_visq <- lm(
  scale(voice_hearing_launayslade, scale = T) ~ 1 + scale(inner_speech, scale = T) * country, #inner_speech is the dialogic subscale
  data = df123)
...
## Note that voice_score here is spiritual voice score, taken by summing together voice questions from spiritual events survey 
m.dp1 <- lm(
